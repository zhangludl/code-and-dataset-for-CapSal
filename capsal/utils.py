...
def resize_bbox(bbox_1f,input_image):
    # oh_masks = (skimage.transform.resize(oh_masks, (512, 512),
    #         order=1, mode="constant", preserve_range=True)).astype(np.uint8)
    bbox_1f = np.squeeze(bbox_1f)
    h = bbox_1f[2] - bbox_1f[0]
    w = bbox_1f[3] - bbox_1f[1]
    c_y = (bbox_1f[2] - h / 2)
    c_x = (bbox_1f[3] - w / 2)
    new_y1 = np.maximum((c_y - np.round(h * 0.75)), 0).astype(np.int32)
    new_x1 = np.maximum((c_x - np.round(w * 0.75)), 0).astype(np.int32)
    new_y2 = np.minimum(c_y + np.round(h * 0.75), input_image.shape[0]).astype(np.int32)
    new_x2 = np.minimum((c_x + np.round(w * 0.75)), input_image.shape[1]).astype(np.int32)
